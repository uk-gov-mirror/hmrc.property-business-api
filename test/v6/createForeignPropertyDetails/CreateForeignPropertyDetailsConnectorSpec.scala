...
class CreateForeignPropertyDetailsConnectorSpec extends ConnectorSpec with Def1_CreateForeignPropertyDetailsFixtures {
  ...
  private val tysTaxYear = "2026-27"
  ...
  "connector" must {
    ...
    "put a valid body and return 204 for a valid tax year" in new HipTest with Test {
      ...
      willPost(
        url = url"$baseUrl/itsd/income-sources/$nino/foreign-property-details/$businessId?taxYear=$tysTaxYear",
        body = requestBody
      ).returns(Future.successful(outcome))

      val result: DownstreamOutcome[Unit] = await(connector.createForeignPropertyDetails(request))
