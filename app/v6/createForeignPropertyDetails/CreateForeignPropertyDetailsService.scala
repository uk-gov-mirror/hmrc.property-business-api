...
@Singleton
class CreateForeignPropertyDetailsService @Inject() (connector: CreateForeignPropertyDetailsConnector) extends BaseService {

  def createForeignPropertyDetails(
      request: CreateForeignPropertyDetailsRequestData)(implicit ctx: RequestContext, ec: ExecutionContext): Future[ServiceOutcome[Unit]] = {

    connector.createForeignPropertyDetails(request).map(_.leftMap(mapDownstreamErrors(downstreamErrorMap)))
