...
class Def1_CreateForeignPropertyDetailsValidator @Inject() (nino: String, businessId: String, taxYear: String, body: JsValue)
  ...
  def validate: Validated[Seq[MtdError], CreateForeignPropertyDetailsRequestData] =
    (
      ...
      resolveJson(body)
    ).mapN(Def1_CreateForeignPropertyDetailsRequestData.apply) andThen validateBusinessRules
